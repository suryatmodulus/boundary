...
// waitConnectionStateType provides some idiomatic expected state
// types for ExpectConnectionStateOnController.
type waitConnectionStateType string

const (
	waitConnectionStateTypeOpen   waitConnectionStateType = "open"
	waitConnectionStateTypeClosed waitConnectionStateType = "closed"
)
...
func testWorkerSessionCleanupSingle(burdenCase timeoutBurdenType) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// Run again, test according to burden case.
		...
		if burdenCase == timeoutBurdenTypeWorker {
			sess.ExpectConnectionStateOnController(ctx, t, c1, waitConnectionStateTypeOpen)
		} else {
			sess.ExpectConnectionStateOnController(ctx, t, c1, waitConnectionStateTypeClosed)
		}
...
func testWorkerSessionCleanupMulti(burdenCase timeoutBurdenType) func(t *testing.T) {
	return func(t *testing.T) {
		...
		c2 := c1.AddClusterControllerMember(t, &controller.TestControllerOpts{
			Logger:                    c1.Config().Logger.Named("c2"),
		...
		// Actual effect depends on burden case.
		if burdenCase == timeoutBurdenTypeController {
			// Burden on controller, should be successful until connection
			// resumes
			sConn.TestSendRecvAll(t)
...
func (s *testSession) ExpectConnectionStateOnController(
	...
	expectState waitConnectionStateType,
) {
	require := require.New(t)
	...
	actualStates := make([]waitConnectionStateType, len(conns))
	// Make expect set for comparison
	expectStates := make([]waitConnectionStateType, len(conns))
	...
	for i, conn := range conns {
		go func(i int, conn *session.Connection) {
			actualState := waitConnectionStateTypeOpen
			for {
				if ctx.Err() != nil {
					break
				}
				...
				for _, state := range states {
					if state.Status == session.StatusClosed {
						actualState = waitConnectionStateTypeClosed
						break
					}
				}

				if actualState == waitConnectionStateTypeClosed {
					break
				}
